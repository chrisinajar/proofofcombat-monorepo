...
Start here. Pick 1–3 runbooks whose “when to use” matches your change by file globs (see each file’s YAML `triggers`), area, or owner.
...
- ts-checks: Run and triage TypeScript checks reliably; handle noisy tests — Use to get parseable tsc output and keep checks green without weakening jest.
...
Maintenance
- After adding or editing runbooks or this index, run `yarn agent:runbook:sync` (updates `.cursor/skills/`).
- Verify no drift with `yarn agent:check`.
- **Replaced system:** runbooks used to sync to `.cursor/rules/*.mdc`; that is **deprecated** in favor of `.cursor/skills/`. Do not reintroduce rules-based sync for runbooks.
