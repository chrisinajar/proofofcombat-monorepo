...
owner: server, ui
triggers:
  - proofofcombat-server/**
  - proofofcombat-ui/**
checklist:
  - Use `yarn --cwd <pkg> ts --pretty false` to get parseable output
  - Pipe output to a file if terminal truncates
  - If tests are noisy, exclude `**/*.test.ts[x]` from tsc (tsconfig `exclude`)
  - Do not change jest/ts-jest config just to make `tsc` happy
source: runbook
---
