...
| 2026-04-08 | ~~**Bug:** `executeAltarBlessing` checks `altar-blessing-emerald` twice; `altar-blessing-sapphire` has no handler — sapphire blessings always fail.~~ **Fixed 2026-04-08.** | proofofcombat-server/schema/locations/npc-shops.ts (`executeAltarBlessing`) | Resolved; second emerald block corrected to sapphire. |
